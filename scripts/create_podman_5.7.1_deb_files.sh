...
DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends devscripts bash-completion conmon \
    build-essential libsubid-dev dh-golang go-md2man golang-go libapparmor-dev libbtrfs-dev libdevmapper-dev debhelper \
    pkg-config libsystemd-dev libgpgme-dev libseccomp-dev libglib2.0-dev golang-dbus-dev
